...
class Attention(torch.nn.Module):
    ...
    def train(self, mode=True):
        ...
        if mode and hasattr(self, 'ab'):
            ...
            self.ab = self.attention_biases[:, self.attention_bias_idxs.to(self.attention_biases.device)]

    def _ensure_device_sync(self):
        """Ensure attention_bias_idxs is on the same device as attention_biases"""
        if hasattr(self, 'attention_bias_idxs') and hasattr(self, 'attention_biases'):
            if self.attention_bias_idxs.device != self.attention_biases.device:
                self.attention_bias_idxs = self.attention_bias_idxs.to(self.attention_biases.device)
        # Recreate ab if it exists to ensure it's on the correct device
        if hasattr(self, 'ab') and not self.training:
            self.ab = self.attention_biases[:, self.attention_bias_idxs]

    def to(self, *args, **kwargs):
        result = super().to(*args, **kwargs)
        result._ensure_device_sync()
        return result

    def forward(self, x):  # x (B,N,C)
        # Ensure device synchronization before computation
        ...
        # Record attention maps only if explicitly enabled (to avoid overhead)
        if getattr(self, 'record_attention', False) and not self.training:
            self.last_attn = attn
...
def model_factory(C, D, X, N, drop_path, weights,
    ...
    if pretrained:
        ...
        if num_classes != 1000:
            # Drop classifier and distillation heads to avoid size mismatch
            for k in list(state.keys()):
                if k.startswith('head.') or k.startswith('head_dist.'):
                    state.pop(k, None)
